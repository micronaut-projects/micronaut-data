...
class DtoSpec extends Specification implements PostgresHibernateReactiveProperties {
    ...
    void "test entity graph"() {
        when:
        def results = bookRepository.findAllByTitleStartsWith("The").collectList().block()
    ...
    void "test no entity graph"() {
        when:
        def results = bookRepository.findAllByTitleStartingWith("The").collectList().block()
