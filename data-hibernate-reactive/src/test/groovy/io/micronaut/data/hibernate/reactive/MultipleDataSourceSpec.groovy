...
class MultipleDataSourceSpec extends Specification implements PostgresWithOtherHibernateReactiveProperties {
    ...
    void "test multiple data sources"() {
        when:
        personRepository.save(new Person(name: "Fred")).block()
        personRepository.save(new Person(name: "Bob")).block()

        then:
        personRepository.count().block() == 2
        otherPersonRepository.count().block() == 0

        when:
        otherPersonRepository.save(new Person(name: "Joe")).block()
