...
class PageSpec extends Specification implements PostgresHibernateReactiveProperties {
    ...
    private void populate() {
        ...
        crudRepository.saveAll(people).collectList().block()
    }

    void "test single result that returns pageable"() {
        when:
        def page = crudRepository.find(10, Pageable.from(1)).block()
    ...
    void "test unpaged pageable with order"() {
        when:
        def results = personRepository.list(Pageable.UNPAGED.order(Sort.Order.asc("name"))).block()
    ...
    void "test pageable list"() {
        ...
        def count = crudRepository.count().block()

        then:"the count is correct"
        count == 1300

        when:"10 people are paged"
        Page<Person> page = personRepository.list(Pageable.from(0, 10)).block()
        ...
        page = crudRepository.findAll(page.nextPageable()).block()
    ...
    void "test pageable findBy"() {
        ...
        Page<Person> page2 = crudRepository.findPeople("A%", pageable).block()
...
//        slice.content
    ...
    void "test total size of find with join"() {
        ...
        when:
        def page = bookRepository.findAll(Pageable.from(0, books.size())).block()
