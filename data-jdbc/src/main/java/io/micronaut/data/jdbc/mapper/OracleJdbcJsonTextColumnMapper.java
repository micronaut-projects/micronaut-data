...
/**
 * The Oracle JDBC json column mapper writing object to string.
 *
 * @author radovanradic
 * @since 4.0.0
 *
...
@Internal
@Experimental
class OracleJdbcJsonTextColumnMapper implements SqlJsonValueMapper {
     ...
     */
    public OracleJdbcJsonTextColumnMapper(OracleJdbcJsonTextObjectMapper textJsonMapper) {
        this.textJsonMapper = textJsonMapper;
    }

    @Override
    public boolean canMapValue(SqlStoredQuery<?, ?> sqlStoredQuery, Object value) {
        // Might need additional checks and annotations later perhaps, for now check only dialect
        if (value == null) {
            return false;
        }
