...
class CompositeSpec extends Specification implements H2TestPropertyProvider {
    ...
    void "test build create Settlement"() {
        when:
            ...
            def statements = encoder.buildCreateTableStatements(null, builder.runtimeEntityRegistry.getEntity(Settlement))
    ...
    void "test build create Citizen"() {
        when:
            QueryBuilder encoder = new SqlQueryBuilder()
            def statements = encoder.buildCreateTableStatements(null, builder.runtimeEntityRegistry.getEntity(Citizen))
