...
class OracleNativeBooleanTypeSpec extends Specification implements OracleTestPropertyProvider {

    @AutoCleanup
    @Shared
    ApplicationContext applicationContext = ApplicationContext.run(properties)

    void "test oracle native boolean type is selected by TypeDef"() {
        given:
        ...
        repository.findByActive(true)*.id() == [trueEntity.id()]
        repository.findByActive(false)*.id() == [falseEntity.id()]
        repository.findByActive(null).empty
