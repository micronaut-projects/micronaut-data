 ...
 */
@Internal
public final class DetachInterceptor<T> extends AbstractQueryInterceptor<T, Void> implements DataInterceptor<T, Void> {

    private final JpaRepositoryOperations jpaRepositoryOperations;

    DetachInterceptor(@NonNull RepositoryOperations operations) {
        super(operations);
    ...
    public Void intercept(RepositoryMethodKey methodKey, MethodInvocationContext<T, Void> context) {
        ...
        detachValue(argument);
        return null;
    }

    private void detachValue(Object value) {
        if (value == null) {
            return;
        }
        jpaRepositoryOperations.detach(value);
    }
}
