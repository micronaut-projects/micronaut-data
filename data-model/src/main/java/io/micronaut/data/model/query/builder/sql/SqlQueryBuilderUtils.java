...
import io.micronaut.data.model.schema.DbType;
...
final class SqlQueryBuilderUtils {
     ...
     */
    static ColumnDefinition getColumn(PersistentProperty prop, String column, boolean primaryKey, boolean required, boolean isForeign) {
        if (prop instanceof Association) {
            throw new IllegalStateException("Association is not supported here");
        }
        AnnotationMetadata annotationMetadata = prop.getAnnotationMetadata();
        ...
        switch (dataType) {
            case STRING:
                ...
                return new ColumnDefinition(column, dataType, dbType, primaryKey, length, required, autoGenerated, generatedValueType, definition);
            case UUID:
            case BOOLEAN:
            case TIMESTAMP:
            case DATE:
            ...
            case JSON:
                ...
                return new ColumnDefinition(column, dataType, dbType, primaryKey, length, precision, scale, required, autoGenerated, generatedValueType,
                    definition, jsonDataType);
            case INTEGER:
                if (optPrecision.isPresent()) {
                    // TODO: Does precision make sense for integer
                    precision = optPrecision.getAsInt();
                }
                return new ColumnDefinition(column, dataType, dbType, primaryKey, length, precision, required, autoGenerated, generatedValueType,
                    definition);
            case BIGDECIMAL:
            case FLOAT:
            case DOUBLE:
     ...
     * @throws MappingException if the data type of the property is unknown
     */
    private static DbType getDbType(PersistentProperty property) {
