...
import io.micronaut.data.model.schema.DbTypes;
...
final class SqlQueryBuilderUtils {
    ...
    ColumnDef getColumn(PersistentProperty prop, String column, Dialect dialect, boolean required) {
        if (prop instanceof Association) {
            throw new IllegalStateException("Association is not supported here");
        }
        AnnotationMetadata annotationMetadata = prop.getAnnotationMetadata();
        String definition = annotationMetadata.stringValue(MappedProperty.class, "definition").orElse(null);
        DataType dataType = prop.getDataType();
        boolean autoGenerated = prop.isGenerated();
        ...
        switch (dataType) {
            case STRING:
                ...
                return new ColumnDef(column, dataType, dbType, length, required, autoGenerated, definition);
            case UUID:
            case BOOLEAN:
            case TIMESTAMP:
            case DATE:
            ...
            case JSON:
                ...
                return new ColumnDef(column, dataType, dbType, length, precision, scale, required, autoGenerated, definition, jsonDataType);
            case INTEGER:
                if (optPrecision.isPresent()) {
                    // TODO: Does precision make sense for integer
                    precision = optPrecision.getAsInt();
                }
                return new ColumnDef(column, dataType, dbType, length, precision, required, autoGenerated, definition);
            case BIGDECIMAL:
            case FLOAT:
            case DOUBLE:
                ...
                return new ColumnDef(column, dataType, dbType, length, precision, scale, required, autoGenerated, definition, null);
            default:
                throw new MappingException("Unable to create table column for property [" + prop.getName() + "] of entity [" + prop.getOwner().getName() + "] with unknown data type: " + dataType);
        }
    }
     ...
     */
    private static int getDbType(PersistentProperty property) {
