...
""")
        ...
        alias() | "name"    | "StartsWithIgnoreCase"   | ["name": String] | "(LOWER(${alias}.$property) LIKE CONCAT(LOWER(:p1),'%'))"
        alias() | "name"    | "EndsWithIgnoreCase"     | ["name": String] | "(LOWER(${alias}.$property) LIKE CONCAT('%',LOWER(:p1)))"
        ...
        alias() | "name"    | "StartingWithIgnoreCase" | ["name": String] | "(LOWER(${alias}.$property) LIKE CONCAT(LOWER(:p1),'%'))"
        alias() | "name"    | "EndingWithIgnoreCase"   | ["name": String] | "(LOWER(${alias}.$property) LIKE CONCAT('%',LOWER(:p1)))"
        ...
        alias() | "name"    | "ContainsIgnoreCase"     | ["name": String] | "(LOWER(${alias}.$property) LIKE CONCAT('%',LOWER(:p1),'%'))"
        alias() | "name"    | "ContainingIgnoreCase"   | ["name": String] | "(LOWER(${alias}.$property) LIKE CONCAT('%',LOWER(:p1),'%'))"
    }

    @Unroll
    void "test #criterion criterion produces the correct distinct query - comparison"() {
        given:
        String methodName = "findDistinctBy${NameUtils.capitalize(property)}${criterion}"
        String sig = signature.entrySet().collect { "$it.value.name $it.key" }.join(",")
        BeanDefinition beanDefinition = buildRepository('test.MyInterface', """
import io.micronaut.data.model.entities.Person;
...
""")
        ...
        expect: "The query is valid"
        query == "SELECT DISTINCT $alias.* FROM $Person.name AS $alias WHERE " + expectedQuery
