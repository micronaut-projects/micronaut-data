...
import io.micronaut.data.model.runtime.convert.vector.impl.VectorTextConvertersFactory
...
class OracleTypeConvertersFactorySpec extends Specification {
    ...
    def "string write converters produce Oracle textual format"() {
        given:
        def factory = new VectorTextConvertersFactory(new JacksonDatabindMapper(new ObjectMapper()))
    ...
    def "string read converters parse Oracle textual format"() {
        ...
        factory.fromStringToByteVector().convert("[1.4, 2.6]", ByteVector, null).get().toByteArray().toList() == [1 as byte, 2 as byte]
    }
}
