 ...
 */
package io.micronaut.data.r2dbc.postgres.json
...
class PostgresJSONSpec extends Specification implements PostgresTestPropertyProvider {
    ...
    SalePgJsonbRepository saleRepository = applicationContext.getBean(SalePgJsonbRepository.class)
    SalePgJsonbItemRepository saleItemRepository = applicationContext.getBean(SalePgJsonbItemRepository.class)
    ...
    void "test read and write json"() {
        when:
            SalePgJsonb sale = new SalePgJsonb()
            sale.setName("test 1")
            sale.data = [foo: 'bar']
    ...
    void "test read write json with string field"() {
        ...
        given:
            def sale = new SalePgJsonb(name: "sale")
    ...
    void "test read and write json with child rows"() {
        given:
            def sale = saleRepository.save(new SalePgJsonb(name: "test 1"))
