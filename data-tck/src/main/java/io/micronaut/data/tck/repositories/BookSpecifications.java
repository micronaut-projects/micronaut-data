...
public final class BookSpecifications {
    ...
    public static final class WithMetamodel {
        public static PredicateSpecification<Book> titleEquals(String title) {
            return (root, criteriaBuilder) -> criteriaBuilder.equal(root.get(Book_.title), title);
        }

        public static PredicateSpecification<Book> titleContains(String title) {
            return (root, criteriaBuilder) -> criteriaBuilder.like(criteriaBuilder.upper(root.get(Book_.title)), "%" + title.toUpperCase() + "%");
        }

        public static PredicateSpecification<Book> titleEqualsWithJoin(String title) {
            return (root, criteriaBuilder) -> {
                ((PersistentEntityFrom<?, ?>) root).join(Book_.GENRE, io.micronaut.data.annotation.Join.Type.LEFT_FETCH);
                return criteriaBuilder.equal(root.get(Book_.title), title);
            };
        }

        public static PredicateSpecification<Book> hasChapter(String chapterTitle) {
            return (root, criteriaBuilder) -> {
                var join = root.join(Book_.chapters);
                return criteriaBuilder.equal(join.get(Chapter_.title), chapterTitle);
            };
        }
        ...
        public static CriteriaQueryBuilder<Book> findUsingASubquery(String name) {
            return new CriteriaQueryBuilder<Book>() {
                ...
                public CriteriaQuery<Book> build(CriteriaBuilder criteriaBuilder) {
                    ...
                    subquery.where(criteriaBuilder.equal(subqueryBookRoot.get(Book_.title), name));
                    criteriaQuery.where(
                        criteriaBuilder.in(bookRoot.get(Book_.id)).value(subquery)
                    );
                    return criteriaQuery;
                }
            };
        ...
        public static PredicateSpecification<Book> titleAndTotalPagesEquals(String title, int totalPages) {
            return (root, criteriaBuilder) -> {
                List<Predicate> predicates = new ArrayList<>(3);
                if (title != null) {
                    predicates.add(criteriaBuilder.equal(root.get(Book_.title), title));
                }
                if (totalPages > 0) {
                    predicates.add(criteriaBuilder.equal(root.get(Book_.totalPages), totalPages));
                }
                predicates.add(criteriaBuilder.isNotNull(root.get(Book_.id)));
                return criteriaBuilder.and(predicates.toArray(new Predicate[0]));
            };
        }
        ...
        public static PredicateSpecification<Book> titleAndTotalPagesEqualsUsingConjunction(String title, int totalPages) {
            return (root, criteriaBuilder) -> {
                ...
                if (title != null) {
                    predicate =
                        criteriaBuilder.and(
                            predicate, criteriaBuilder.equal(root.get(Book_.title), title));
                }
                if (totalPages > 0) {
                    predicate =
                        criteriaBuilder.and(
                            predicate,
                            criteriaBuilder.equal(
                                root.get(Book_.totalPages), totalPages));
                }
                predicate =
                    criteriaBuilder.and(
                        predicate, criteriaBuilder.isNotNull(root.get(Book_.id)));
                return predicate;
            };
        }
