...
public class ProductManager {
    ...
    public ProductManager(HibernateReactorRepositoryOperations operations) {
    ...
    Mono<Product> find(String name) {
        return operations.withSession(session -> Mono.fromCompletionStage(session.createQuery("from Product p where p.name = :name", Product.class)
