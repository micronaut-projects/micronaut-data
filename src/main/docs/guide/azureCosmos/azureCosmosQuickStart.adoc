...
Next up you need to configure at least one data source. The following snippet from `application.yml` is an example of configuring the default Azure Cosmos Db data source:
...
In Azure Cosmos world, every `MappedEntity` will correspond to the container. One container can hold only one entity or document type.
`MappedEntity` name will be used to obtain container name. If entity class is `CosmosBook` then expected container name will be `cosmos_book` unless not overriden in `MappedEntity` annotation value. Default naming strategy for entity fields is `Raw` strategy and users should not usually need to override it.
