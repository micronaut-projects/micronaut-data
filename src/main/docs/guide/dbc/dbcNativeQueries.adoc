...
For Oracle raw `@Query` methods with a `RETURNING` clause, you can write either `RETURNING id, name` or the full Oracle form `RETURNING id, name INTO ?, ?`. If `INTO` is omitted, Micronaut Data generates the Oracle `INTO` placeholders automatically; if `INTO` is present, it must use positional `?` placeholders.
