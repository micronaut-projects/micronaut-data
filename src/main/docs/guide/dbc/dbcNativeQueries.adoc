...
NOTE: For Oracle raw `@Query` methods that use `INSERT`, `UPDATE`, or `DELETE` with a `RETURNING` clause, define the full SQL yourself, including the explicit `RETURNING column1, column2 INTO ?, ?` list.
