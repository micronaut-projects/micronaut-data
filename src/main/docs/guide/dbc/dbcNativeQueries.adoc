...
For Oracle raw `@Query` methods with a `RETURNING` clause, write the returned columns and `INTO` placeholders explicitly, for example `RETURNING id, name INTO ?, ?`. The `INTO` clause must use positional `?` placeholders. You can also provide the full `BEGIN ... END;` block yourself.
