...
NOTE: For Oracle raw `@Query` statements that use `RETURNING`, omit the `INTO` clause. Micronaut Data generates the Oracle `INTO` bindings automatically. Writing `RETURNING ... INTO ...` directly in the query is not supported.
