...
=== Connection client info tracing (Oracle)

In order to trace SQL calls using `java.sql.Connection.setClientInfo(String, String)` method, you can
annotate repository with @api:data.connection.annotation.OracleConnectionClientInfo[] annotation. If boolean field `disableClientInfoTracing` is set to true, then Micronaut will not issue `setClientInfo` calls to the JDBC connection.
...
These values will be set to the JDBC connection using `setClientInfo("OCSID.MODULE", module)` and `setClientInfo("OCSID.ACTION", action)` respectively.
...
Please note this is currently supported only for Oracle database connections.
