...
annotate repository with @api:data.connection.annotation.Connectable[] annotation. If boolean field `traceClientInfo` is not set to true, then Micronaut will not issue `setClientInfo` calls to the JDBC connection.
