...
NOTE: Unlike the JPA implementation there is no dirty checking. `save` inserts entities without an identity value and updates entities with a generated or always auto-populated identity value already present. Entities with non-generated assigned identities are inserted by default. Use `insert` to require an insert, and use an `update` method for explicit or partial updates.
