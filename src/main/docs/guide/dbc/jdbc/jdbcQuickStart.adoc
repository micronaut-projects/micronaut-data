...
NOTE: Unlike the JPA implementation there is no dirty checking. `save` decides between a SQL `INSERT` and `UPDATE` based on the entity identity value. Use `insert` to require an insert, and use an `update` method for explicit or partial updates.
