...
NOTE: For PostgreSQL vector support with R2DBC, Micronaut Data uses `io.r2dbc.postgresql.codec.Vector` from the PostgreSQL R2DBC driver. The pgvector Java library is only required when using JDBC pgvector driver-native types; sparse `PGsparsevec` mapping is currently JDBC-only.
