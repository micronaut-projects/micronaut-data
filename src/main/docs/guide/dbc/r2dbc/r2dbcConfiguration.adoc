...
NOTE: For Oracle sparse `VECTOR(..., SPARSE)` usage with R2DBC, include a recent Oracle JDBC driver as well so Oracle sparse VECTOR APIs are available at runtime.

dependency:com.oracle.database.jdbc:ojdbc11[scope="runtimeOnly"]

NOTE: For PostgreSQL vector support, include the pgvector Java library at runtime so Postgres driver vector types are available. In Micronaut Data, dense `vector` is supported for JDBC and R2DBC; sparse `PGsparsevec` mapping is currently JDBC-only.
...
Please note that currently schema validation is not supported for Micronaut Data R2DBC.
