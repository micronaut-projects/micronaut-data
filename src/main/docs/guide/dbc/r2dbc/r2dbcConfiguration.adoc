...
NOTE: For PostgreSQL `vector`/`sparsevec` support, include the pgvector Java library at runtime so the Postgres driver types (`PGvector` for dense vectors and `PGsparsevec` for sparse vectors) are available for both JDBC and R2DBC binding paths.
