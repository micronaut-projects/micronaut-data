As mentioned in the <<jdbcQuickStart, Quick Start>> section, if you need to customize how entities map to the table and column names of the database you can use JPA annotations to do so or Micronaut Data's own annotations in the `io.micronaut.data.annotation` package.
...
snippet::example.Country[project-base="doc-examples/jdbc-example", source="main", tags="country"]
