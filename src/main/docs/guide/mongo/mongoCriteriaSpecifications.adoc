...
For MongoDB-specific criteria queries, Micronaut Data also supports:

- text search predicates (`$text`)
- geospatial predicates (`$geoWithin`, `$geoIntersects`, `$near`, `$nearSphere`)

See query usage details and examples in xref:mongo/mongoCriteriaSpecifications/mongoCriteriaExecuteQuery.adoc[Mongo criteria execute query].

NOTE: Negated `$text`, `$near`, and `$nearSphere` predicates are not supported.
