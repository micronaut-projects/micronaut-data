...
NOTE: MongoDB text search requires a text index on the queried collection (for example ann:data.mongodb.annotation.index.MongoTextIndexed[]).
...
NOTE: Geospatial operators require compatible geospatial indexes and data modeling (for example ann:data.mongodb.annotation.index.MongoGeoIndexed[]).
