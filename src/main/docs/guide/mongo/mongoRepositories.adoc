...
Micronaut Data MongoDB also supports geospatial repository finder methods and criteria predicates for MongoDB GeoJSON values.
MongoDB repositories should use the MongoDB Java driver's geospatial types from `com.mongodb.client.model.geojson`, for example
`Point`, `LineString`, and `Polygon`.
The following repository method fragments are supported:

* `findByPointGeoWithin(geometry)`
* `findByPointGeoIntersects(geometry)`
* `findByPointNear(geometry, distance)`

Micronaut Data translates these operations to MongoDB's geospatial query operators:

[cols="1,1"]
|===
| Predicate | MongoDB query operator

| `GeoWithin`
| `$geoWithin` with `$geometry`

| `GeoIntersects`
| `$geoIntersects` with `$geometry`

| `Near`
| `$near` with `$geometry` and `$maxDistance`
|===

`Near` expects the second argument to be a numeric distance value and writes it to MongoDB as `$maxDistance`.
