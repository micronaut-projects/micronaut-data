...
The default value is `ReturnDocument.BEFORE`.
If you want the updated state returned, use `returnDocument = ReturnDocument.AFTER` explicitly.
...
For update methods returning `void`, `boolean`, `int`, or `long`, `returnDocument` has no effect because those methods execute count/acknowledgement update operations.
