...
When the repository method returns an entity/DTO, Micronaut Data MongoDB will execute the update as a single-document update and map the returned document.
To control whether the document is returned before or after the update, use the `returnDocument` member of ann:data.mongodb.annotation.MongoUpdateOptions[].
...
NOTE: MongoDB can only return a single updated document for an update operation (using `findOneAndUpdate`). Update-returning methods must use a single-result return type (for example `Entity`, `Mono<Entity>`, or `CompletionStage<Entity>`).
