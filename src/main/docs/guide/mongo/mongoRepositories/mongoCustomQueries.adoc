...
Update-returning methods must use a single-result return type and ann:data.mongodb.annotation.MongoUpdateReturningQuery[] (including supported single-item reactive/future types such as `Mono<T>` and `CompletionStage<T>`).
...
NOTE: Filter, update and pipeline JSON can reference method parameters. The `project` and `sort` values must be constant JSON; `:param`-style placeholders are not supported there.
