...
NOTE: MongoDB can only return a single updated document for an update operation (using `findOneAndUpdate`). Update-returning methods must use a single-result return type (for example `Entity` or `Mono<Entity>`).
