...
NOTE: MongoDB can only return a single updated document for an update operation (using `findOneAndUpdate`). Update-returning methods must use a single blocking result type (for example `Entity`).
