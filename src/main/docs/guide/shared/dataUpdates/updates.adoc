To require an update of an existing entity, pass the entity to the `update` method:

snippet::example.BookRepository[project-base="doc-examples/hibernate-example", source="main", tags="update", indent="0"]

Use `save` when you want Micronaut Data to decide between insert and update based on the entity identity value.
Use `update` when the entity must already exist.
