...
The built-in CRUD repository interfaces provide three distinct write operations:
`save` / `saveAll` perform a conditional save, inserting entities without an identity value and updating entities with a generated or always auto-populated identity value already present.
Entities with non-generated assigned identities are inserted by default.
`insert` / `insertAll` require an insert operation.
`update` / `updateAll` require an update operation.

To temporarily restore the Micronaut Data 4 behavior where `save` and `saveAll` always issue insert operations, set `micronaut.data.save-as-insert` to `true`.
For new code, prefer `insert` and `insertAll` when an insert is required.
