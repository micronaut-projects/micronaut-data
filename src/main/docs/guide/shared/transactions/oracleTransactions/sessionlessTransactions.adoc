...
The feature is intended for top-level workflow boundaries. `SUSPEND` and `REQUIRES_SUSPENDED` must start their own
transaction: if a parent transaction is already active the invocation fails, because a participating method would otherwise
write into the parent transaction while the suspended Oracle transaction is left behind.
The sessionless modes require the default `REQUIRED` propagation and cannot be combined with other propagation values.

Sessionless transactions are applied by transactional advice, so they are available only on synchronous methods annotated
with ann:transaction.annotation.OracleTransactional[]. Reactive and `CompletionStage` methods are rejected.
...
For a method annotated with `@OracleTransactional(sessionless = SUSPEND)`, Micronaut Data:

* requires an active Oracle sessionless propagation state
* begins the JDBC transaction normally, using the regular JDBC transaction manager
* starts an Oracle sessionless transaction with `OracleConnection.startTransaction(...)` on the transactional connection
* suspends the Oracle transaction with `OracleConnection.suspendTransactionImmediately()` immediately before the JDBC commit,
which detaches it from the session so that the commit has nothing left to commit
* stores the returned Oracle global transaction identifier, or GTRID, in the propagation state once the suspend succeeded
...
For a method annotated with `@OracleTransactional(sessionless = REQUIRES_SUSPENDED)`, Micronaut Data:
