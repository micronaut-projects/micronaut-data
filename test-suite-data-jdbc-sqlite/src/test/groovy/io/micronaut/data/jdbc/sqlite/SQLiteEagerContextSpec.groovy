...
class SQLiteEagerContextSpec extends Specification implements SQLiteTestPropertyProvider {

    @AutoCleanup
    @Shared
    ApplicationContext context = ApplicationContext.builder(getProperties() + ['eager-test': true] + getSQLiteDataSourceProperties("other"))
    ...
    void "test eager start"() {
        when:
            def personRepository = context.getBean(SQLitePersonRepository)
        then:
