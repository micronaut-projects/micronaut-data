...
class SQLiteJSONSpec extends AbstractJSONSpec implements SQLiteTestPropertyProvider {

    @Memoized
    @Override
    SaleRepository getSaleRepository() {
        return applicationContext.getBean(SQLiteSaleRepository)
    }

    @Memoized
    @Override
    SaleItemRepository getSaleItemRepository() {
        return applicationContext.getBean(SQLiteSaleItemRepository)
    }

    @Memoized
    @Override
    JsonEntityRepository getJsonEntityRepository() {
        return applicationContext.getBean(SQLiteJsonEntityRepository)
    }
}
