...
class SQLiteReactiveRepositorySpec extends AbstractReactiveRepositorySpec implements SQLiteTestPropertyProvider {

    @Memoized
    @Override
    PersonReactiveRepository getPersonRepository() {
        return context.getBean(SQLiteReactivePersonRepository)
    }

    @Memoized
    @Override
    StudentReactiveRepository getStudentRepository() {
        return context.getBean(SQLiteStudentReactiveRepository)
    }

    @Memoized
    @Override
    BookReactiveRepository getBookRepository() {
        return context.getBean(SQLiteReactiveBookRepository)
    }
}
